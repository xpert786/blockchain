  ...
  return (
    <div className="min-h-screen bg-[#F4F6F5]">
      ...
      <main className="w-full px-6 py-8">
        ...
        <h1 className="text-3xl font-medium text-[#0A2A2E] font-poppins-custom">Messages</h1>
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <aside className="bg-white border border-[#E5E7EB] rounded-3xl p-6 w-full lg:w-1/3">
            <div className="mb-4">
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-full bg-[#E5F1F0] flex items-center justify-center text-sm font-medium text-[#0A2A2E]">
                        {initials}
                      </div>
                      <div className="flex-1">
                        <div className="flex items-center justify-between">
                          <p className="text-sm font-medium text-[#0A2A2E] font-poppins-custom">{conversation.name}</p>
                          <span className="text-xs text-[#748A91] font-poppins-custom">{conversation.lastActive}</span>
                        </div>
          ...
          <section className="bg-white border border-[#E5E7EB] rounded-3xl p-6 flex-1 flex flex-col min-h-[70vh]">
            <header className="flex items-center justify-between border-b border-[#E5E7EB] pb-4 mb-4">
              <div>
                <h2 className="text-lg font-medium text-[#0A2A2E] font-poppins-custom">{selectedConversation.name}</h2>
                <p className="text-xs text-[#748A91] font-poppins-custom">
                  Syndicate Lead · {selectedConversation.status}
                </p>
              </div>
              <div className="text-xs text-[#748A91] font-poppins-custom">Last active {selectedConversation.lastActive}</div>
            </header>
              ...
              {selectedMessages.map((message) => {
                ...
                return (
                  <div key={message.id} className={`flex ${isOwn ? "justify-end" : "justify-start"}`}>
                    <div
                      className={`max-w-lg rounded-2xl px-4 py-3 text-sm font-poppins-custom shadow-sm ${
                        isOwn ? "bg-[#D7F8F0] text-[#0A2A2E]" : "bg-[#F4F6F5] text-[#0A2A2E]"
                      }`}
                    >
                      <p className="mb-2">{message.content}</p>
                      <span className="text-xs text-[#748A91]">{message.time}</span>
                    </div>
                  </div>
                );
              })}
